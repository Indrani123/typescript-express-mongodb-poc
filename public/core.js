...
var Stores_1 = require("../typescript-express-mongodb/model/Stores");
var MasterStore_1 = require("./model/MasterStore");

var StoresApp = angular.module('StoresApp', ["ngRoute"])
...
StoresApp.controller('addRestaurant',function ($scope,$http) {
	...
	$scope.addRestaurant=function (addStoreName,addStoreLocation,addZipCode,addUserLoc,addMiles) {
		var store={};
		var userLocation=[];
		var masterStores=[];
		var request=[];
		...
		request.push(MasterStore);
		console.log(JSON.stringify(request));
		$http.post('/addUpdateStore',request,[])
			.then(function (response) {
				$scope.addResponse(response.data[0]);
			});
	}

	$scope.addReponse=function (res) {
		console.log(res);
	}
});

StoresApp.controller('login',function ($scope,$http) {
